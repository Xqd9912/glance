...
6. 前端渲染。
   ...
   - 默认颜色沿用中心原子颜色，opacity 低一些，例如 0.18-0.28；边线可以是同色加深或中性灰。
...
- 输出稳定 ID、中心 atom ID、vertex atom IDs、face indices、颜色、visibility dependency groups。
...
这里 `faces` 的索引指向 `vertexAtomIds`，前端通过 `atomById` 拿坐标。这样 atoms toggle 关掉时，polyhedra 仍然可以画；但 boundary / one-hop toggles 关掉时，可以按 vertex IDs 正确过滤。
...
- 新增 `Polyhedron` render component：根据 `vertexAtomIds` 找坐标，用 `faces` 构造 `BufferGeometry`，再画 surface 和 edge overlay。
...
- 前端材质、透明度、edge style，以及和 atoms/bonds 的渲染顺序。
...
6. 再考虑中心元素选择、opacity/style、ChemEnv label。
