...
app = typer.Typer(
    help="Glance command line tools.",
...
def gui(
    ...
    """Start the local Glance GUI server."""
    selected_port = _choose_port(host, port)
    url = f"http://{host}:{selected_port}"

    typer.echo(f"Starting Glance GUI at {url}")
    if not no_open:
        _start_browser_opener(url, host, selected_port)

    if reload:
        uvicorn.run(
            "glance.server.app:create_app",
            host=host,
            port=selected_port,
            factory=True,
            reload=True,
