...
from glance.server.routes import router
from glance.server.security import ApiTokenMiddleware
...
def create_app(
    ...
    `glance gui` calls this with no token and no extra origins: the page is served from the
    ...
    """
    app = FastAPI(title="Glance", version="0.1.0")
    ...
    if allow_origins:
        app.add_middleware(
            ...
            # headers such as x-glance-filename, and a preflight that omits any one
...
def _resolve_static_root(
    ...
    candidates.append(Path(str(resources.files("glance") / "web_static")))
